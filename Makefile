...
local.input=input
local.output=output_large_m_5000_s_3
local.log=log_large_m_5000_s_3
...
local: jar clean-local-output
	spark-submit --class ${job.name} --master ${local.master} --name "${app.name}" ${jar.name} ${local.input} ${local.output} ${threshold} ${source}
...
aws: jar upload-app-aws delete-output-aws
	aws emr create-cluster \
		--name "Spark SSSP Group 30 large cluster M=5000 s=3" \
